...
class TestOpenRouterIntegration(unittest.TestCase):
    ...
    def test_chat_offline_on_connection_blocked(self):
        """Provedor com chave, mas rede bloqueada → resposta graciosa offline, não erro cru."""
    ...
    def test_chat_offline_when_no_provider_configured(self):
        """Nenhuma chave de sistema nem do usuário → resposta graciosa offline (missão §45/62)."""
        import nemo_server as ns
        from fastapi.testclient import TestClient

        import uuid
        user, token = ns.AUTH_STORE.register("Test NoKey", f"nokey-{uuid.uuid4().hex[:8]}@test.local", "senha123")
        headers = {"Authorization": f"Bearer {token}"}
        ...
        with patch.object(ns.AI_SERVICE, "has_system_key", return_value=False), \
            ...
            resp = tc.post(
                "/api/nemo/chat",
                json={"agent": "analista", "message": "oi", "messages": []},
                headers=headers,
            )
            self.assertEqual(resp.status_code, 200)
            data = resp.json()
            self.assertTrue(data["ok"])
            self.assertTrue(data["offline"])
            self.assertIn("Nenhum provedor", data["content"])
